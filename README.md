...
| Tapa: Con espigas / Plana / Deslizante / Con bisagra / Con bisagra doble | Con espigas | "Con espigas": la tapa encaja dentro como el resto de las piezas. "Plana": un rectángulo liso que se apoya sobre el canto de las paredes; el alto total y el espacio interior no cambian. Con la casilla "Agarradera" se le agrega una perilla armada de dos piezas en domo (encastradas a 90°, mismo grosor que el material) que atraviesa dos agujeros en cruz — uno en la tapa, otro en un inserto pegado por debajo, del tamaño exacto del hueco interno de la caja. "Deslizante": la tapa corre dentro de un canal vaciado en laterales y fondo. "Con bisagra": la tapa queda unida por una bisagra hecha del mismo material — una espiga cuadrada maciza sale de cada canto lateral de la tapa, cerca de la esquina trasera, y encaja en un agujero de su lateral — y gira sobre el canto trasero (que queda limpio, sin nada cortado ahí) hasta pegar contra el fondo. Las cuatro paredes miden lo mismo que con la tapa plana, salvo un poste que sobresale en la esquina trasera de cada lateral para dejarle material alrededor al agujero. "Con bisagra doble": la tapa se parte en dos mitades iguales, cada una con su propia bisagra cerca de su propio lateral, girando sobre un eje de adelante hacia atrás — se abren como dos alas y se encuentran en el medio. Acá las paredes frontal y trasera quedan a la altura completa (las mitades encajan a ras, no apoyadas encima), con dos postes y dos agujeros cada una (uno por mitad) y una muesca compartida en el medio para las dos lengüetas, que sostienen la tapa cerrada donde se encuentran. |
...
| Tamaño de la espiga (mm) | el grosor del material | Solo con tapa con bisagra (simple o doble). Lado del cuadrado macizo que hace de eje de la bisagra — no un diámetro, un cuadrado. Con "Automático" marcada sigue al grosor del material (una espiga de 3 mm para 3 mm de material). |
| Holgura del agujero (mm) | 1 | Solo con tapa con bisagra (simple o doble). Cuánto más grande se dibuja el agujero que la espiga necesita para girar sin trabarse (el agujero sale de la diagonal de la espiga, no de su lado, porque al girar la espiga barre un círculo del tamaño de esa diagonal). Se calibra con una prueba antes de cortar la caja: dura si es poca, bamboleante si es mucha. |
| Ancho de la manija (mm) | 20 | Solo con tapa con bisagra (simple o doble). Ancho de la lengüeta que sobresale del canto delantero de la tapa y de la muesca a juego en la pared frontal. Con la bisagra doble, es el ancho TOTAL de la muesca — cada mitad aporta la mitad de ese ancho desde su lado. |
| Profundidad de la manija (mm) | grosor + 5 | Solo con tapa con bisagra (simple o doble). Cuánto sobresale la lengüeta más allá del perímetro de la caja — crece con el grosor del material para que siempre quede cómoda de agarrar. Con "Automática" marcada sigue esa fórmula. |
...
- `js/boxes/` — los tipos de caja concretos: `simple-box.js` (caja cerrada y tapa plana), `sliding-box.js` (tapa deslizante), `hinged-box.js` (tapa con bisagra), `hinged-double-box.js` (tapa con bisagra doble, reutiliza varios parámetros de `hinged-box.js`), `lid-grip.js` (agarradera de la tapa plana), `dividers.js` (divisores internos: columnas, capas y su ranura de cruce, reutilizado por los cuatro tipos de tapa) y `shared.js` con lo que todos comparten.
...
- Tope de apertura diseñado a un ángulo específico en la tapa con bisagra (simple o doble).
- Bisagra con varilla separada (dowel o metal) en vez de espiga integral.
- Refuerzo redondeado (gusset) en la base de la lengüeta de la bisagra doble.
